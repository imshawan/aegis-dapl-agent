...
**The Open-Source AI On-Call Engineer that fixes production bugs while you sleep.**
...
## Table of Contents
- [The 3 AM PagerDuty Problem](#the-3-am-pagerduty-problem)
...
## The 3 AM PagerDuty Problem

You're asleep. A critical service goes down. PagerDuty wakes you up. You spend 45 minutes digging through Sentry stack traces, pulling up Git blame, and trying to understand a microservice you didn't even write.

**Aegis is the solution.** It acts as an autonomous SRE agent that intercepts the alert, debugs the codebase in real-time, and opens a Pull Request with the fix, often before you even open your laptop.

Built upon a **Dynamic Agentic Planning Loop (DAPL)**, Aegis ingests error stack frames, isolates AST code boundaries around failure lines, evaluates recent Git blame history, and formulates debugging hypotheses via iterative ReAct tool orchestration to open verified remediation pull requests directly in version control.

---

## Quickstart (Under 2 Minutes)

Aegis is designed to be frictionless. Start the orchestrator, Redis, and MongoDB locally to see it work.
...
# 2. Set up your AI keys (Supports Gemini, Claude, GPT-4, or local Ollama)
cp .env.example .env

# 3. Build and launch the agent
npm install
npm run build
npm start
```
*Note: Make sure Redis and MongoDB are running locally or via Docker for BullMQ task scheduling.*
...
For a complete list of all supported environment variables (including advanced tuning for distributed locking, multi-model LLM failover, and AWS Secrets Manager integration), please refer to the **[Environment Variables Matrix](./docs/environment-variables.md)**.
