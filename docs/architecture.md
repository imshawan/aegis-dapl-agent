...
- **Ingestion & Path Normalization**: When an incident webhook arrives (Sentry, Slack, or raw traceback), normalizers extract the failing file path (`filePath`) and the exact production version reference (`resolvedRef`)—prioritizing the commit SHA, falling back to release tag, and defaulting to branch name. *(See [API Ingestion Payloads](./api-payloads.md) for detailed JSON schemas).*
...
- **AST Scoping via Local Clone**: The `CodeScoperWorker` invokes `WorkspaceManager` to read the repository content from the local clone mapped to the incident `jobId`. It extracts a target AST window of $\pm20$ lines around the failure line, and caches the snippet in Redis using MD5 checksum hashing to eliminate redundant disk reads across loop turns.
...
To protect the autonomous agent from adversarial manipulation, denial of service, directory traversal, credential leakage, and unauthorized access, Aegis implements an impenetrable security shielding service (`src/security/`) that intercepts and sanitizes all external data before it reaches the queue or LLM evaluation loops:
- **Zero-Trust Webhook Authentication (`validateWebhookAccessKey`)**: Intercepts all POST requests to `/api/v1/webhooks/*` (except health checks), enforcing valid access key tokens in the `accesskey` header before payload parsing or LLM token expenditure.
- **Automated AWS Secrets Manager Rotation (`SecretsManagerService`)**: Directly integrates with AWS Secrets Manager using cloud-native IAM Roles for Service Accounts (IRSA) and hourly background polling (`AccessKeyService.startAwsSecretRotationPolling`), enabling zero-downtime key rotation without restarting containers. Supports secondary fallback to `AEGIS_ACCESS_KEYS` for Kubernetes External Secrets Operator (ESO) workflows.
